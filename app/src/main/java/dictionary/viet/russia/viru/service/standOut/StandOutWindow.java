package dictionary.viet.russia.viru.service.standOut;
...
import dictionary.viet.russia.viru.configuration.StandOutFlags;
import dictionary.viet.russia.viru.event.SearchPopupEvent;
import dictionary.viet.russia.viru.event.clickEvent.ClickMenuPopupEvent;
...
public abstract class StandOutWindow extends Service {
    ...
    public static void show(Context context,
                            Class<? extends StandOutWindow> cls, int id, String word) {
        if (word == null || word.isEmpty()) {
            context.startService(getShowIntent(context, cls, id));
        } else {
            context.startService(getShowIntent(context, cls, id, word));
        }
    ...
    public static Intent getShowIntent(Context context,
        ...
        String action = cached ? ACTION_RESTORE : ACTION_SHOW;
        Uri uri = cached ? Uri.parse("standout://" + cls + '/' + id) : null;
        return new Intent(context, cls).putExtra("id", id).setAction(action).putExtra("word", word)
                .setData(uri);
    }
...
//    }
     ...
     * time {@link #show(int, String)} is called.
     *
     * @param id The id of the window shown.
...
//    }
    ...
    public PopupWindow getDropDown(final int id) {
        ...
        items.add(new DropDownListItem(R.mipmap.ic_zoom,
            ...
            @Override
            public void run() {
                EventBus.getDefault().post(new ClickMenuPopupEvent(ClickMenuPopupEvent.TYPE_ZOOM));
            }
        }));

        items.add(new DropDownListItem(R.mipmap.ic_hide,
                getString(R.string.hide), new Runnable() {

            @Override
            public void run() {
                EventBus.getDefault().post(new ClickMenuPopupEvent(ClickMenuPopupEvent.TYPE_HIDE));
            }
        }));
     ...
     */
    public final synchronized Window show(int id, String word) {
...
//            window = cachedWindow;
//        } else {
//            window = new Window(this, id, word);
//        }
        window = new Window(this, id, word);
...
//                    notification);
    ...
    public final void setIcon(int id, int drawableRes) {
        ...
        if (window != null) {
            ...
            if (icon instanceof ImageView) {
                ((ImageView) icon).setImageResource(R.mipmap.ic_menu);
            }
        }
    }
    ...
    @Subscribe
    public void onEvent(SearchPopupEvent event) {
        CLog.d(TAG, "seach poup event1 : " + event.word);
//        EventBus.getDefault().post(new ChangeWordPopup(event.word));
